...
import com.example.childcare.adapters.MessageAdapter
import com.example.childcare.databinding.ActivityViewMessagesBinding
import com.example.childcare.models.Message
...
class ViewMessages : AppCompatActivity() {
    ...
    private fun messageListener(roomId: String) {
        FirebaseDatabase.getInstance().getReference("Messages/${roomId}").addValueEventListener(object :
            ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                messages.clear()
